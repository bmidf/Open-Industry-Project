...
## Murr-style field I/O block. Exposes a single BOOL "communication fault"
## tag that auto-faults on a random interval for a random duration to
## simulate a fieldbus blip. While faulted, `LED_Dome` glows red; otherwise
## it sits green. Mirrors `APF`'s comms-fault pulse pattern but with both
## the wait time and the pulse width drawn from configurable ranges.

const LED_OK_COLOR: Color = Color(0, 1, 0, 1)
const LED_FAULTED_COLOR: Color = Color(1, 0, 0, 1)
const LED_EMISSION_ENERGY: float = 2.0

## Live communication-fault state. true = PLC sees the FIO as missing,
...
## HIGH while the FIO is auto-faulted, LOW while healthy.
@export var communication_fault_tag_name: String = ""


@onready var _led_dome: MeshInstance3D = find_child("LED_Dome", true, false) as MeshInstance3D
